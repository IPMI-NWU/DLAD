...
if __name__ =="__main__":
    ...
    stabilize= False
    next_idx = 1
    next_expand_epoch = [0, 50, 100, 200, 350, 550, 800, 1100]
    for epoch in range(800, opt.n_epochs):
        ...
        for i, (image, _, _) in enumerate(train_loader):
            ...
            real_out = net_Enc(real_image, expand, alpha)
            ...
            loss_D = loss_D_real + 0.5 * (loss_D_rec + loss_D_gen)
            loss_D.backward()
            optimizer_D.step()
            ...
            loss_LD = 0.5 * (loss_LD_guss + loss_LD_real)
            loss_LD.backward()
            optimizer_LD.step()
            ...
            rec_image = net_Dec(real_latent.detach(), expand, alpha)
            ...
            loss_Enc = loss_Enc_rec + loss_Enc_Vec_Clf + loss_Enc_GAN
            ...
            loss_Dec = loss_Dec_rec + 0.5 * (loss_Dec_GAN_gen + loss_Dec_GAN_rec)
